...
import { Trophy, Zap, Flame, TrendingUp, Calendar, AlertCircle, ChevronRight, BookOpen, Trash2, Edit, Check, Target, ClipboardList, Star, Crown, Medal } from 'lucide-react';
import { Topic, Simulado } from './types';
import { AREAS, getLevelInfo, getTodayStr, getStreak, formatDate, getAreaTheme, getPerformanceColor, calculateNextLoad } from './utils';
...
export const EvolutionChart = ({ simulados, targetAccuracy }: { simulados: Simulado[], targetAccuracy: number }) => {
    ...
    // Increased padding to ensure tooltips and high points aren't cut off
    const paddingY = 15; 
    ...
    const getY = (val: number) => {
        ...
        // Map to 100-padding to padding (inverted Y axis)
    ...
    const areaPath = data.length > 1 ? `${dPath} L ${100} 150 L 0 150 Z` : '';
    const targetY = getY(targetAccuracy);

    return (
        <div className="w-full h-full relative group/chart select-none pt-4">
            <svg viewBox="0 0 100 100" className="absolute inset-0 w-full h-full overflow-visible" preserveAspectRatio="none">
                ...
                {[60, 80, 100].map(val => (
                    <line key={val} x1="0" y1={getY(val)} x2="100" y2={getY(val)} stroke="currentColor" className="text-slate-200 dark:text-white/5" strokeWidth="1" strokeDasharray="4" vectorEffect="non-scaling-stroke"/>
                ))}
            ...
            <div className="absolute inset-0 w-full h-full pointer-events-none">
                {data.map((point, i) => {
                    ...
                    // Logic to flip tooltip if it's too high up
                    const isHigh = top < 30;
            ...
            <div className="flex justify-between items-start mb-4">
                <div className="flex items-center gap-3">
                    ...
                    <div>
                        ...
                        <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider mt-0.5">{topic.area}</p>
                    </div>
                </div>
