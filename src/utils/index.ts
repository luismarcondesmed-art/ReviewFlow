...
export const AREAS: { id: AreaType; name: string; full: string }[] = [
  { id: 'geral', name: 'Geral', full: 'Conhecimentos Gerais' },
  { id: 'tecnico', name: 'Técnico', full: 'Conhecimentos Técnicos' },
  { id: 'idiomas', name: 'Idiomas', full: 'Línguas e Idiomas' },
  { id: 'exatas', name: 'Exatas', full: 'Ciências Exatas' },
  { id: 'humanas', name: 'Humanas', full: 'Ciências Humanas' },
];

export const SUB_AREAS: Record<AreaType, string[]> = {
  geral: ['Atualidades', 'Cultura', 'Artes'],
  tecnico: ['Programação', 'Direito', 'Administração', 'Engenharia', 'Saúde'],
  idiomas: ['Inglês', 'Espanhol', 'Francês', 'Alemão', 'Mandarim'],
...
export const calculateFSRSNextDate = (accuracy: number, previousIntervalDays: number): string => {
    ...
    // 1. Grade mapping (1-4)
    let grade = 1; // Again
    if (accuracy >= 90) grade = 4; // Easy
    else if (accuracy >= 70) grade = 3; // Good
    else if (accuracy >= 50) grade = 2; // Hard

    // 2. Estimate D (Difficulty) based on accuracy (assumed baseline D=5 for a medium topic)
    let D = 5;
    if (grade === 4) D = 3;
    if (grade === 3) D = 5;
    if (grade === 2) D = 7;
    ...
    // 3. Current Stability (S) is approximately the previous interval 
    const currentS = Math.max(1, previousIntervalDays);

    let nextS = currentS;
    if (grade === 1) {
        // Lapsing (Forgetting): Drastic stability penalty
        nextS = Math.max(1, currentS * 0.2); 
    } else {
        // Success: Stability increases. 
        // Spacing effect: The longer the previous interval, the more stable the memory builds.
        const expDifficulty = Math.exp(1 - D / 5); // Easier = higher growth
        const spacingEffect = 1 / Math.log(currentS + 1.5);
        
        let gradeMultiplier = 1;
        if(grade === 2) gradeMultiplier = 1.3;
        if(grade === 3) gradeMultiplier = 2.0;
        if(grade === 4) gradeMultiplier = 2.8;

        nextS = currentS * (1 + expDifficulty * spacingEffect) * gradeMultiplier;
    }

    // 4. Fuzzing (Random variance +/- 5% to prevent review clumping)
    const fuzz = 1 + (Math.random() * 0.1 - 0.05);
    nextS *= fuzz;
    
    // 5. Target Interval (Targeting R = 90% retention).
    let nextInterval = Math.max(1, Math.round(nextS));
    
    // Hard cap at 180 days to prevent infinite forgetting in extreme cases
...
export const getAreaTheme = (areaId: AreaType) => {
  switch (areaId) {
    case 'geral': return { bg: 'bg-slate-100 dark:bg-slate-500/10', text: 'text-slate-700 dark:text-slate-300', border: 'border-slate-200 dark:border-slate-500/20', activeBorder: 'border-slate-500', iconBg: 'bg-slate-500', chartFill: '#64748b' };
    case 'tecnico': return { bg: 'bg-red-100 dark:bg-red-500/10', text: 'text-red-700 dark:text-red-300', border: 'border-red-200 dark:border-red-500/20', activeBorder: 'border-red-500', iconBg: 'bg-red-500', chartFill: '#ef4444' };
    case 'idiomas': return { bg: 'bg-amber-100 dark:bg-amber-500/10', text: 'text-amber-700 dark:text-amber-300', border: 'border-amber-200 dark:border-amber-500/20', activeBorder: 'border-amber-500', iconBg: 'bg-amber-500', chartFill: '#f59e0b' };
    case 'exatas': return { bg: 'bg-purple-100 dark:bg-purple-500/10', text: 'text-purple-700 dark:text-purple-300', border: 'border-purple-200 dark:border-purple-500/20', activeBorder: 'border-purple-500', iconBg: 'bg-purple-500', chartFill: '#a855f7' };
    case 'humanas': return { bg: 'bg-emerald-100 dark:bg-emerald-500/10', text: 'text-emerald-700 dark:text-emerald-300', border: 'border-emerald-200 dark:border-emerald-500/20', activeBorder: 'border-emerald-500', iconBg: 'bg-emerald-500', chartFill: '#10b981' };
...
export const getTopicTrack = (topic: Topic): 'residencia' | 'concurso' | 'geral' => {
    if (topic.targetTrack) return topic.targetTrack;
    
    const src = (topic.source || '').toLowerCase();
    const title = (topic.title || '').toLowerCase();
    const subarea = (topic.subarea || '').toLowerCase();
    const tags = (topic.tags || []).map(t => t.toLowerCase());

    const isConcursoMatch = 
        src.includes('fafipa') || 
        src.includes('concurso') || 
        title.includes('fafipa') ||
        title.includes('edital') ||
        tags.some(t => t.includes('fafipa') || t.includes('concurso')) ||
        subarea.includes('portuguesa') ||
        subarea.includes('matemática') ||
        subarea.includes('informática') ||
        subarea.includes('legislação');

    if (isConcursoMatch) return 'concurso';

    const isResidenciaMatch = 
        src.includes('medcof') || 
        src.includes('estrategia') || 
        src.includes('medreview') || 
        src.includes('residencia') ||
        tags.some(t => t.includes('residencia') || t.includes('r1'));

    if (isResidenciaMatch) return 'residencia';

    return 'geral';
};

export const getTrackBadgeInfo = (track: 'residencia' | 'concurso' | 'geral') => {
    switch (track) {
        case 'concurso':
            return {
                label: 'Concurso FAFIPA',
                shortLabel: 'Concurso',
