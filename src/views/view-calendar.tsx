...
import { CalendarCheck, ChevronLeft, ChevronRight, ClipboardList, Flag, BookOpen, CheckCircle2, Calendar as CalendarIcon, Clock, Link as LinkIcon, ExternalLink, Plus, List, Grid } from 'lucide-react';
import { Topic, Simulado, UserConfig } from '../types';
import { getTodayStr, getAreaTheme } from '../utils';
import { useCalendar } from '../hooks';

export const CalendarView = ({ topics, simulados, onOpenReview, config, onUpdateTopic }: { topics: Topic[], simulados: Simulado[], onOpenReview: (id: string, idx: number) => void, config: UserConfig, onUpdateTopic?: (topic: Topic) => void }) => {
    ...
    const handleAddNotionLink = (topicId: string) => {
        ...
        const newLink = prompt('Insira o link do Notion para este tema:', currentLink);
        
        if (newLink !== null) {
            onUpdateTopic({ ...topic, notionLink: newLink });
        }
    };
        ...
        <div className="h-full flex flex-col pb-32 lg:pb-0 animate-scale-in max-w-6xl mx-auto w-full">
            ...
            <div className="flex flex-row items-center justify-between gap-3 mb-6 px-2">
                ...
                <div className="flex items-center gap-2">
                    {/* Mobile View Toggles */}
                    <div className="lg:hidden flex bg-slate-100 dark:bg-white/5 p-1 rounded-xl">
                        <button onClick={() => setMobileViewMode('agenda')} className={`p-2 rounded-lg transition-all ${mobileViewMode === 'agenda' ? 'bg-white dark:bg-zinc-800 shadow-sm text-blue-500' : 'text-slate-400'}`}><CalendarIcon size={16}/></button>
                        <button onClick={() => setMobileViewMode('calendar')} className={`p-2 rounded-lg transition-all ${mobileViewMode === 'calendar' ? 'bg-white dark:bg-zinc-800 shadow-sm text-blue-500' : 'text-slate-400'}`}><Grid size={16}/></button>
                        <button onClick={() => setMobileViewMode('list')} className={`p-2 rounded-lg transition-all ${mobileViewMode === 'list' ? 'bg-white dark:bg-zinc-800 shadow-sm text-blue-500' : 'text-slate-400'}`}><List size={16}/></button>
                    </div>
            ...
            <div className="flex flex-col lg:flex-row gap-6 flex-1 min-h-0">
                        ...
                        <div className="space-y-4 mb-4">
                            {!hasEventsInMonth ? (
                                ...
                                Array.from({ length: daysInMonth }).map((_, i) => {
                                    ...
                                    return (
                                        <div key={dateStr} className="bg-white dark:bg-zinc-900 rounded-2xl p-4 border border-slate-100 dark:border-white/5">
                                            <div className="flex items-center gap-2 mb-3">
                                                <div className="w-8 h-8 rounded-lg bg-blue-100 dark:bg-blue-900/30 flex items-center justify-center text-blue-600 font-bold">
                                                    {day}
                                                </div>
                                            ...
                                            <div className="space-y-2">
                                                {dayData.reviews.map((r: any) => (
                                                    <div key={r.topicId + r.idx} className="flex items-center gap-2 text-sm text-slate-600 dark:text-slate-400">
                                                        <div className={`w-2 h-2 rounded-full ${r.done ? 'bg-emerald-500' : 'bg-blue-500'}`}></div>
                                                        <span className={r.done ? 'line-through opacity-50' : ''}>{r.topicTitle}</span>
                                                    </div>
                                                ))}
                                            </div>
                                        </div>
                ...
                {(mobileViewMode === 'agenda' || window.innerWidth >= 1024) && (
                            ...
                            {selectedDayData.reviews.length === 0 && selectedDayData.sims.length === 0 && !isSelectedExam ? (
                                ...
                                <>
                                    ...
                                    {selectedDayData.reviews.sort((a: any, b: any) => (a.done === b.done ? 0 : a.done ? 1 : -1)).map((r: any) => {
                                         ...
                                         return (
                                            <div key={r.topicId + r.idx} className={`flex flex-col p-4 bg-white dark:bg-zinc-800/50 rounded-2xl border shadow-sm transition-all gap-3
                                                    ...
                                                    <div className="flex items-center gap-2">
                                                        ...
                                                        {notionLink ? (
                                                            ...
                                                            <button 
                                                                ...
                                                                className="p-2 bg-slate-100 dark:bg-white/10 text-slate-400 hover:text-blue-500 rounded-xl transition-colors"
                                                                title="Adicionar Link Notion"
                                                            >
                                                                <LinkIcon size={16}/>
                                                            </button>
                                                        )}
